 ...
 */
    ...
    <div class="container-fluid">
        ...
        <form action="{{ route('panel.partner.account.index') }}" id="serviceCategoryForm" method="GET" id="TableForm">
            <div class="row">
                <!-- start message area-->
                <!-- end message area-->
                <div class="col-md-12">
                        ...
                        <div class="card-header d-flex flex-wrap justify-content-between">
                            ...
                            <div class="d-flex flex-wrap justify-content-right">
                                ...
                                @if($bankDetails->count() <= 0)
                                    <a href="{{ route('panel.partner.account.create') }}" class="btn btn-icon btn-sm btn-outline-primary mr-2" title="Add New"><i class="fa fa-plus" aria-hidden="true"></i></a>
                                @endif
                                {{-- <a href="{{ route('panel.partner.account.index') }}" id="reset" data-url="{{ route('panel.partner.account.index') }}" class="btn btn-icon btn-sm btn-outline-danger mr-2" title="Reset"><i class="fa fa-redo" aria-hidden="true"></i></a> --}}
                            </div>
                        </div>
                        <div id="ajax-container">
                            @include('backend.partner.account.load')
                        </div>
                    </div>
                </div>
            </div>
